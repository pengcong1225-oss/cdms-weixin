# cdms-weixin：CDMS + RWFit 患者端小程序

本项目为纯原生微信小程序，不使用微信云开发（CloudBase）：仓库内没有 `cloudfunctions/`、`cloudbaserc.json` 和云环境配置，CDMS/RWFit 代码全部位于 `miniprogram/`。患者端基于 RWFit 官方 Demo，负责手环扫描、绑定、历史同步和数据上传；医生端通过 `pages/wearable/sync/index` 负责一次性实时检测。

发布边界：`D:\weixinProject\cdms-weixin` 是完整业务小程序的唯一发布工程；`cdms-iot/miniapp` 仅作为 IoT 登录与采集桥接参考，不单独发布，也不覆盖本项目的页面路由。

## 工程配置

- `project.config.json` 的 `miniprogramRoot` 为 `miniprogram/`、`compileType` 为 `miniprogram`；项目不使用云开发，因此没有 `cloudfunctionRoot`。
- 发布包默认使用 `miniprogram/config/runtime.js` 中的生产地址；开发者工具启动参数仍可覆盖它们，因此体验版不依赖私有联调配置。
- 所有服务端交互均走 HTTPS（CDMS、cdmsManager、cdms-iot 三套 Base URL）；小程序不持有 IoT 密钥，设备会话由服务端签发。

## 运行链路

1. 登录方式：患者使用「姓名 + 已建档手机号」登录（`cdmsBaseUrl` 调用 `cdms` 的 `POST /api/v1/miniapp/auth/login`，body 为 `{ realName, phone }`），医生使用 CDMS 账号 + 密码登录；服务端返回患者/医生角色并支持个人中心切换。患者端不再走微信授权，也不保存或提交账户密码。
...
5. 小程序初始化 BLE 适配器，使用 handoff 兑换短期 IoT 会话。
